...
**Operator UI.** FAROS is now the primary operator interface, providing a configurable workspace composed of custom widgets and operators. The workspace includes: a **Process ID Provider** operator for process selection, a **Timeseries Monitor** widget for live plotting of geometry plus profile-defined process parameters, a **Measurement KPI** widget for key metrics display, a **Deviation Alert** widget for real-time alerting, and **Process Controls** for start/stop operations. In the current welding profile, the primary process parameters are wire feed speed, travel speed, and arc length correction; current and voltage remain available as auxiliary measured telemetry when published by the source. These widgets communicate via the Alert Engine's existing JSON endpoints and query Mintaka's temporal API for historical data. A legacy static HTML dashboard remains available for development and testing purposes.
...
* **Process** is the anchor entity. It records the operation mode (parameter‑driven or geometry‑driven) and profile-defined process/AI inputs. For the current welding baseline, the AI input contract is `wire_feed_speed_mpm_model_input`, `travel_speed_mps_model_input`, and `arc_length_correction_mm_model_input`. It also stores a `toleranceThreshold` so the operator can change sensitivity without redeploying code.
...
**Parameter‑driven path.** The operator (or UI) chooses the model input parameters; in the current welding profile these are wire feed speed, travel speed, and arc length correction. The engine uses the forward model to generate an expected geometry. Measurements come in, and we compare measured height/width against expected height/width. If either exceeds the tolerance percentage, we open an alert and offer next actions. The “AI retry” action calls the inverse path to propose a small parameter adjustment and stores it as a new AIRecommendation.
...
The FAROS workspace provides a professional operator interface with drag-and-drop configurability. The **Process ID Provider** operator manages process selection and creation, feeding process IDs to connected widgets. The **Timeseries Monitor** widget plots geometry and profile-defined process parameters from measurement data via the Alert Engine's `/process/{id}/measurements` endpoint, which queries Mintaka's temporal API. For welding, that means bead height, bead width, wire feed speed, travel speed, and arc length correction as the primary live parameters, with current and voltage still usable as auxiliary telemetry if available. The **Measurement KPI** widget displays current values and deviation percentages. The **Deviation Alert** widget polls `/check-deviation` and presents operator actions when thresholds are exceeded. The **Process Controls** widget manages process lifecycle (start/stop/resume) via the Alert Engine's process management endpoints.
