...
def find_latest_fronius_for_bead(
    bead_id: str,
    fronius_samples: Iterable[FroniusTelemetrySample],
    max_progression_for_fallback: float = 0.05,
) -> Optional[FroniusTelemetrySample]:
    """
    Fallback: return the most recently appended fronius sample for *bead_id*
    when all samples for that bead are stuck near progression=0 (i.e. TF
    lookup failed during the welding pass and the progression node published
    0.0 throughout).

    Only activates when *every* fronius sample for the bead has progression
    below *max_progression_for_fallback* (default 5 %).  This avoids
    returning a misleading sample in normal operation, where fronius
    progression spans the full 0-to-1 range.
    """
    matching: list[FroniusTelemetrySample] = []
    for sample in fronius_samples:
        if sample.bead_id == bead_id:
            matching.append(sample)

    if not matching:
        return None

    # Only fall back if ALL samples are near zero — TF-failure signature
    if any(s.progression > max_progression_for_fallback for s in matching):
        return None

    # Deque iterates oldest → newest; last element is the most recent
    return matching[-1]
