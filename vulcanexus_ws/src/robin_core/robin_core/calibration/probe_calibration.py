"""Probe and CTWD calibration — touch-sensing based surface finding."""
...
from robin_interfaces.srv import (
    FindSurface, CalibrateWireTip, CalibratePlatePlane,
    SetCtwd, SetFloat32 as SetFloat32Srv,
)

from robin_core.utils.utils import wait_for_future, set_wago_signal_direct
from robin_core.plate_geometry import normalize_corner_id, inward_to_world


class ProbeCalibration:
    """Touch-sensing surface probing and CTWD calibration.
    ...
    def __init__(self, node, cb_group, controller_name: str):
        ...
        self._set_ctwd_client = node.create_client(SetCtwd, "/tcp/set_ctwd")
    ...
    def _ensure_trajectory_control_mode(self) -> tuple[bool, str]:
        ...
        traj_ctrl = self._controller_name
    ...
    def find_surface(self, request, response, planner, end_effector_link,
        ...
        from robin_core.bead_layout import compute_weld_velocity_scaling
            ...
            for idx, (px, py) in enumerate(xy_points):
                ...
                for attempt in range(1 + MAX_RETRIES):
                    ...
                    if attempt < MAX_RETRIES:
                        log.warn(
                            f"Probe point {idx + 1}/4 attempt {attempt + 1} "
                            f"failed — retrying…")
                        time.sleep(0.5)
                ...
                if not point_ok:
                    ...
                    response.message = (
                        ...
                        f"after {1 + MAX_RETRIES} attempts: {fs_res.message}")
                    return response
    ...
    # -- CalibrateWireTip service implementation --------------------------
    def calibrate_wire_tip(self, request, response, planner,
                           end_effector_link, base_frame, approach_height,
                           current_ctwd, tcp_tf_buffer,
                           max_cart_vel, default_acc_scaling,
                           target_wire_tip: float = 0.0):
        """Service callback implementation for /calibration/calibrate_wire_tip."""
        from robin_core.motion import MotionPlanner

        # Use explicit target if provided, otherwise fall back to current
        effective_wire_tip = target_wire_tip if target_wire_tip > 0.0 else current_ctwd

        log = self._node.get_logger()
        log.info(
            f"CalibrateWireTip: ({request.x:.3f}, {request.y:.3f}), "
            f"surface_z={request.surface_z:.4f}, "
            f"retract={request.retract_length * 1000:.1f}mm, "
            f"target_wire_tip={effective_wire_tip * 1000:.1f}mm "
            f"(requested={target_wire_tip * 1000:.1f}, current={current_ctwd * 1000:.1f})")
        ...
        if self._check_abort("calibrate_wire_tip_start"):
            response.success = False
            response.message = "Calibration aborted"
            return response
        ...
        if not mode_ok:
            response.success = False
            response.message = f"Cannot calibrate wire tip in jog mode: {mode_msg}"
            return response
        ...
        try:
            ...
            if self._check_abort("calibrate_wire_tip_before_retract"):
                response.success = False
                response.message = "Calibration aborted"
                return response
            ...
            if self._check_abort("calibrate_wire_tip_after_retract"):
                response.success = False
                response.message = "Calibration aborted"
                return response

            cal_z = surface_z + max(0.0, float(effective_wire_tip))
            ...
            if self._check_abort("calibrate_wire_tip_after_approach"):
                response.success = False
                response.message = "Calibration aborted"
                return response
            ...
            if self._check_abort("calibrate_wire_tip_before_feed"):
                response.success = False
                response.message = "Calibration aborted"
                return response

            log.info("Enabling touch sensing…")
            ok, err = self._enable_touch_sensing_mandatory("calibrate_wire_tip")
            if not ok:
                response.success = False
                response.message = err
                return response

            self._prepare_touch_monitoring("calibrate_wire_tip")
            ...
            if self._wire_feed_client.wait_for_service(timeout_sec=2.0):
                ...
                if result is None or not result.success:
                    ...
                    self._disable_touch_sensing("calibrate_wire_tip: wire feed failed")
                    response.success = False
                    response.message = f"Wire feed until touch failed: {msg}"
                    return response
            else:
                self._disable_touch_sensing("calibrate_wire_tip: wire feed unavailable")
                response.success = False
                response.message = "Wire feed service not available"
                return response

            if self._check_abort("calibrate_wire_tip_after_feed"):
                self._disable_touch_sensing("calibrate_wire_tip: aborted after feed")
                response.success = False
                response.message = "Calibration aborted"
                return response

            # The wire tip distance is determined by construction: we
            # positioned contact_tip at surface_z + effective_wire_tip,
            # then fed wire until touch.  Use the target value directly
            # — TF-based measurement introduces positioning error.
            measured_wire_tip = float(max(0.0, effective_wire_tip))

            # Sanity-check: verify nozzle is roughly where expected
            try:
                ...
                tf_wire_tip = float(nozzle_z - surface_z)
                deviation = abs(tf_wire_tip - measured_wire_tip) * 1000.0
                if deviation > 1.0:
                    log.warn(
                        f"TF sanity check: nozzle Z implies "
                        f"{tf_wire_tip * 1000:.2f}mm wire tip "
                        f"(deviation {deviation:.1f}mm from target "
                        f"{measured_wire_tip * 1000:.2f}mm)")
                if tf_wire_tip < -0.001:
                    log.warn(
                        f"contact_tip appears below surface by "
                        f"{abs(tf_wire_tip) * 1000:.2f}mm — "
                        f"check surface_z reference")
            except Exception as e:
                log.warn(f"TF sanity check failed (non-fatal): {e}")

            log.info(f"Calibrated wire tip: {measured_wire_tip * 1000:.2f}mm")

            self._disable_touch_sensing("calibrate_wire_tip: measurement complete")

            if self._set_ctwd_client.wait_for_service(timeout_sec=2.0):
                ctwd_req = SetCtwd.Request()
                ctwd_req.data = float(measured_wire_tip)
                ctwd_req.calibrated = True
                future = self._set_ctwd_client.call_async(ctwd_req)
                result = wait_for_future(self._node, future, timeout_sec=5.0)
                if result and result.success:
                    log.info(f"TCP manager updated: wire_tip={measured_wire_tip * 1000:.2f}mm")
                else:
                    log.warn("Failed to update tcp_manager wire tip")
            else:
                log.warn("/tcp/set_ctwd service not available")
            ...
            response.success = True
            response.measured_wire_tip = measured_wire_tip
            response.message = f"Wire tip calibrated: {measured_wire_tip * 1000:.2f}mm"
            log.info(response.message)

        except Exception as e:
            log.error(f"CalibrateWireTip failed: {e}")
            self._disable_touch_sensing("calibrate_wire_tip: exception")
            response.success = False
            response.measured_wire_tip = 0.0
            response.message = str(e)
