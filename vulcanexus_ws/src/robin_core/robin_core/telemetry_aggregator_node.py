...
from robin_core.telemetry_alignment import (
    AlignedTelemetrySample,
    clamp_progression,
    find_latest_fronius_for_bead,
...
class TelemetryAggregatorNode(Node):
    ...
    def __init__(self):
        ...
        self._fronius_sub = self.create_subscription(
            ...
            qos_profile_sensor_data,  # BEST_EFFORT matches weld_data_node publisher
            raw=True,
            callback_group=self._callback_group,
        )
    ...
    def _resolve_pending_geometry(self) -> None:
        ...
        for pending_geometry in self._pending_geometry_history:
            ...
            if fronius_match is None and geometry_sample.bead_id:
                # Fallback: when TF fails during the welding pass, all fronius
                # samples are stuck at progression=0 and stamp_ns=0, so
                # progression-based matching never succeeds.  Use the last
                # fronius sample for the same bead as a representative reading.
                fronius_match = find_latest_fronius_for_bead(
                    geometry_sample.bead_id,
                    self._fronius_history,
                )
                if fronius_match is not None:
                    self.get_logger().info(
                        f'[AGG] Fallback bead-level match for bead '
                        f'{geometry_sample.bead_id} '
                        f'(progression-based match unavailable)'
                    )
            if fronius_match is not None:
    ...
    def _fronius_raw_callback(self, msg) -> None:
        """Deserialize fronius bytes as WelderData and match by progression."""
        try:
            sample = deserialize_message(bytes(msg), WelderData)
    ...
    def _geometry_raw_callback(self, msg) -> None:
        """Deserialize geometry bytes as BeadGeometry and assign progression."""
        try:
            sample = deserialize_message(bytes(msg), BeadGeometry)
        except Exception as exc:
            self.get_logger().debug(f'Geometry deserialise error: {exc}')
            return
        ...
        self._pending_geometry_history.append(
            PendingGeometrySample(
                ...
                cross_sectional_area=0.0,  # not in current BeadGeometry.msg
            )
        )
        self._resolve_pending_geometry()
