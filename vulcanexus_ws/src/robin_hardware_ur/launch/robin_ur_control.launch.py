"""ROBIN fork of ur_control.launch.py — adds OnProcessExit gating.

Forked from ur_robot_driver (Jazzy, eprosima/vulcanexus:jazzy-desktop-4.3.1)
to gain Python references to controller spawner processes.  This lets
robin_main.launch.py attach OnProcessExit handlers for event-driven staging.

When used standalone (ros2 launch robin_hardware_ur robin_ur_control.launch.py)
the OnProcessExit handler just logs a message.  When included from
robin_main.launch.py the parent attaches its own OnProcessExit on the
trajectory_spawner returned by launch_setup().

Upstream source:
  /opt/ros/jazzy/share/ur_robot_driver/launch/ur_control.launch.py
  Copyright (c) 2021 PickNik, Inc. — BSD-3-Clause
"""
...
def launch_setup(context):
    ...
    trajectory_until_node = Node(
        ...
        output="screen",
        parameters=[{"motion_controller": initial_joint_controller}],
    )
    ...
    # ── OnProcessExit: log when controllers are ready ────────────────
    controllers_ready = RegisterEventHandler(
        OnProcessExit(
            target_action=active_spawner,
            on_exit=[
                LogInfo(msg="UR controllers spawned successfully"),
            ],
        )
    )
...
def generate_launch_description():
    declared_arguments = [
        ...
        DeclareLaunchArgument(
            ...
            description="Launchfile providing the robot_state_publisher.",
        ),
        ...
        DeclareLaunchArgument(
            "controller_spawner_timeout", default_value="10",
