...
echo
echo "SANITY CHECKS (run separately):"
echo "  TimescaleDB : python3 scripts/timescale_plot_sanity.py ${PROCESS_ID}"
echo "  Mintaka     : curl -s -G '${MINTAKA_URL}/temporal/entities/${ENTITY_ID}' \\"
echo "                  --data-urlencode 'attrs=urn:robin:processTelemetry' \\"
echo "                  --data-urlencode 'timerel=between' \\"
echo "                  --data-urlencode 'timeAt=1970-01-01T00:00:00Z' \\"
echo "                  --data-urlencode 'endTimeAt=2030-01-01T00:00:00Z' \\"
echo "                  --data-urlencode 'options=temporalValues' \\"
echo "                  --data-urlencode 'lastN=5' | jq"
echo
