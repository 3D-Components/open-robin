...
## 1. MCCP-UQ Inference Engine (`faros/inference/mccp_inference.py`)
...
## 2. MCCP DevLab API (`faros/services/mccp_inference_devlab_server.py`)
...
## 3. Viser 3D Server (`faros/services/viser_server.py`)
...
## 5. React Frontend (`faros/frontend/`)
...
- A small reference `.h5` model and `.pkl` state should be committed to
  `faros/tests/fixtures/` (or generated by a fixture script) so that inference
