...
import { Activity, Bot, Shield, Database, FlaskConical, History, Settings, ChevronRight } from 'lucide-react';
...
export function Sidebar({ tab, setTab }: SidebarProps) {
    const items: Array<{ key: TabKey; label: string; icon: React.ReactNode; hint: string }> = [
        ...
        { key: "robots", label: "Robots", icon: <Bot className="h-4 w-4" />, hint: "Per-robot" },
        { key: "models", label: "Models & Trust", icon: <Shield className="h-4 w-4" />, hint: "AI models" },
        { key: "mlops", label: "MLOps", icon: <Database className="h-4 w-4" />, hint: "Orchestrator" },
        { key: "inference", label: "Uncertainty", icon: <FlaskConical className="h-4 w-4" />, hint: "UQ testbed" },
